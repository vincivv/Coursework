...
char convertChar (char cin)
	...
	if (c < 32) return lowchar;
	if (c > 126) return highchar;
	return (cin);
	}
		...
		for (int i = 0; i < loops; i++)				//Loop for each "Block" within one buffer read
			...
			for (int j = 0; j < BLOCKSIZE/16; j++)	//loop j lines for each block
				...
				if (offset + 16 > readbytes)
					...
					for (k = 0; k < readbytes - offset; k++)
						{
						printf ("%c", convertChar(buf[offset + k]));
						}
					printf("\n");
					...
					printf ("%06X: %02X %02X %02X %02X %02X %02X %02X %02X  %02X %02X %02X %02X %02X %02X %02X %02X | %c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c\n",
						...
						convertChar(buf[offset + 0]),
						convertChar(buf[offset + 1]),
						convertChar(buf[offset + 2]),
						convertChar(buf[offset + 3]),
						convertChar(buf[offset + 4]),
						convertChar(buf[offset + 5]),
						convertChar(buf[offset + 6]),
						convertChar(buf[offset + 7]),
						convertChar(buf[offset + 8]),
						convertChar(buf[offset + 9]),
						convertChar(buf[offset + 10]),
						convertChar(buf[offset + 11]),
						convertChar(buf[offset + 12]),
						convertChar(buf[offset + 13]),
						convertChar(buf[offset + 14]),
						convertChar(buf[offset + 15]));
					}
...
int processArguments (int argc, char * argv[])
	...
	while (1) 
		...
		static struct option long_options[] = {
		   ...
		   {"lowchar",	required_argument, 0, 'l'},	 //forces to l
		   {"highchar",	required_argument, 0, 'x'},	 //forces to x
		...
		c = getopt_long(argc, argv, "c:s:f:l:x:vh",
		...
		switch (c) {
			...
			case 'h':
				printf ("USAGE: hexdump --file <filename> [--count num512ByteBlocks] [--start start512ByteBlock] [--lowchar c] [--highchar c] [--help] [--version]\n");
			    exit (0);
