...
function renderGames(games) {
    ...
    games.forEach((game) => {
        ...
        if (gameId)
            gameId.textContent = `Uno Game #${String(game.id)}`;
        if (creator)
            creator.textContent = `Created by: ${game.creator_email}`;
        if (players)
            players.textContent = `${String(game.player_count)} player(s)`;
        if (status)
            status.textContent = `Status: ${String(game.status)}`;
